...
export class QuizManager {
    ...
    start = async (bot: TelegramBot, input: CallbackQuery) => {
    ...
    proceed = async (bot: TelegramBot, input: CallbackQuery) => {
        const id = input.message?.chat.id
        ...
        //update psyche
        const succ = await this.userService.setPsyche(String(id), attr as PsycheAttributes, Number(val))
        if (!succ) {
            return
        }
        //send result
        ...
        //send
        if (hobbies.length <= 0) {
            await bot.editMessageText(TEXT.quiz.none, {
                chat_id: id,
                message_id: input.message?.message_id,
    ...
    ask() {
        return {
            text: this.prompt, options: {
                reply_markup: new InlineKeyboard(...this.options.map((option: any) =>
